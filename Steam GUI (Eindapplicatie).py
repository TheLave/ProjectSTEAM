	...
	def __init__(self, master, data):
		...
		def raspi_scherm_tonen(geklikte_knop, gekozen_pagina):
			...
			def apa102(clock_pin, data_pin, color):
				"""
				Stuurt het zenden van bytes naar de apa102 aan.
				"""
			...
			def check_personstate(steamid):
				"""
				Checkt de personastate van het ingevoerde steamid.
				"""
				kleuren = {
					'blue': [255, 0, 0],
					'green': [0, 255, 0],
			...
			def pulse(pin, delay1, delay2):
				"""
				Stuurt een pulse naar de Servo.
				"""
			...
			def servo_pulse(pin_nr, position):
				"""
				Aansturing van het sturen van pulsen naar de Servo.
				"""
			...
			def check_state(steamid):
				"""
				Checkt de personastate van een ingevoerd steamid voor de Servo positie.
				"""
				with urllib.request.urlopen("https://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key"
											f"=948437B690B388BBEFF1D07D68AB2553&steamids={steamid}") as url:
					rawData = json.loads(url.read().decode())
			...
			def check_achievments(data):
				"""
				Checkt het aantal behaalde achievements van een ingevoerd steamid voor een ingevoerd appid.
				"""
				voltooid = 0
			...
			def bereken_lampjes(aantalAchievements, data):
				"""
				Berekend hoeveel lampjes moeten branden voor het aantal behaalde achievements.
				"""
				aantalLampjes = -1
				keer = 1
			...
			def zet_lampjes_aan(steamid2, appid):
				"""
				Zet de lampjes aan voor het aantal behaalde achievements.
				"""
			...
			def start_display(steamid):
				"""
				Stuurt het display aan met de gegevens van het ingevoerde steamid.
				"""
				entryIndex = 0
				pressed = False
		...
		def scherm_ti_conf(knop1, knop2, knop3, knop4):
			"""
			Configureerd het raspi scherm voor Tkinter.
			"""
